"""Lomb-Scargle implementation based on the NFFT"""
import numpy as np
...
def lombscargle_nfft(t, y, dy, f0, df, Nf,
    ...
    """Fast Lomb-Scargle Periodogram using the NFFT algorithm
    ...
    exponential_sum_method : string (optional, default='auto')
        method to use for the exponential sum
    ...
    .. [2] Leroy, B. 2012, Fast calculation of the Lomb-Scargle periodogram
           using nonequispaced fast Fourier transforms. A&A 545, A50
    """
    if dy is None:
        dy = 1
    ...
    kwds = dict(f0=f0, df=df, Nf=Nf, method=exponential_sum_method)
    sums = [complex_exponential_sum(t, w * y, **kwds),
            complex_exponential_sum(2 * t, w, **kwds)]
    Sh, Ch = sums[0].imag, sums[0].real
    S2, C2 = sums[1].imag, sums[1].real

    if fit_mean:
        sums.append(complex_exponential_sum(t, w, **kwds))
        S, C = sums[2].imag, sums[2].real
